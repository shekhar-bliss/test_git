<?php
  ...
  require_once(__dir__.'/../header_without.php');
  // Include Pagination
  ...
  $data_igf_server = array(
                      ...
                      array('key' => 'igf_server_data_ip_1', 'name' => 'DATA IP 1', 'width' => '125', 'db_col_name' => 'igf_server_data_ip_1'),
  ...
  if(isset($_GET['search'])) {
    ...
    if (isset($_GET['req_igf_server_data_ip']) && $_GET['req_igf_server_data_ip'] != '') {
      ...
      $sql_where .= " AND ( iser.igf_server_data_ip_1 LIKE '%".mysql_real_escape_string($req_igf_server_data_ip)."%' OR iser.igf_server_data_ip_2 LIKE '%".mysql_real_escape_string($req_igf_server_data_ip)."%' )";
    }

    if(isset($_GET['req_sh']) && $_GET['req_sh'] != '') {
      $req_sh = $_GET['req_sh'];
      $sql_where .= " AND iser.igf_server_server_hall_id = '".mysql_real_escape_string($req_sh)."' ";
    }
  }
  ...
  $sql_total = "SELECT iser.igf_server_id
                FROM
  ...
  $sql_order_by = ' ORDER BY iser.igf_id DESC, iser.igf_server_id ';
  $sql_search = "SELECT iser.*, r.req_id
                 FROM
                  idc_igf_server AS iser
                  LEFT JOIN
                  idc_igf AS i ON iser.igf_id = i.igf_id
...
?>
    <div class="container">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Serial Number Search</li>
      </ol>
      <!-- /breadcrumb  -->
...
?>
      <form class="form-inline" method="GET">
        ...
        <div class="form-group">
          <label class="sr-only" for="req_igf_server_data_ip">Data IP</label>
          <input type="text" class="form-control input-sm" name="req_igf_server_data_ip" id="req-igf-server-data-ip" placeholder="Data IP" size="40" value="<?php echo $req_igf_server_data_ip;?>">
        </div>
